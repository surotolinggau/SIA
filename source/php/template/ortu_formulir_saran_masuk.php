<div id="formulir_ortu_saran_masuk" class="admin_formulir_muncul1">
	<div id="guru_formulir_lihat_kelas">
		...
		<div class="info_kelas">
			<h2 id="judul_tambah_kelas">Kritik dan Saran Yang Masuk</h2>
		</div>
		<div class="bungkus_title">
			<p id="detail_masukan">Formulir Daftar Kritik dan Saran Yang Masuk</p>
		</div>
			...
			<div class="bungkus_kelas">
				<table id="tabel_masukan">
					...
					<?php 
						$querysaranmasuk = "SELECT * FROM masukan WHERE kepada ='$nama_pengguna' ORDER BY tanggal_masukan, jam_masukan DESC";
						$sqlisaranmasuk = mysqli_query($con, $querysaranmasuk);
						$no_masukan = 0;
						if(mysqli_num_rows($sqlisaranmasuk) < 1){?>
						<tr>
							<td colspan="7" id="kosong_saran">Krikik dan Saran Yang Masuk Kosong</td>
						</tr>
						<?php }else{
					...
					<tr>
						...
						<td><?php echo $tampilsaranmasuk['pengirim'];?></td>
						...
						<td>
						<?php 
							if(!empty($tampilsaranmasuk['tanggapan'])){echo $tampilsaranmasuk['tanggapan'];}else{?>
						<a href="?id_sdk=<?php echo $tampilsaranmasuk['id_masukan'];?>#formulir_ortu_tanggapi" id="beritanggapan">Beri Tanggapan</a><?php } ?>
						</td>
					</tr>
					<?php } } ?>
				</table>
